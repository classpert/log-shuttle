...
then
  ...
  # remove container name and color escape sequences
	unbuffer curl -sSN ${LOGSPOUT_URL} | unbuffer -p cut -f2 -d '|' | unbuffer -p sed 's/\x1b\[[0-9;]*m//g' | /bin/log-shuttle -logs-url=${LOGPLEX_INPUT_URL} -logplex-token=${CHANNEL_TOKEN}
fi